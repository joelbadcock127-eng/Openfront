...
list: the continuous-world Oceania map (all of Oceania as one playable
world — generated by `npm run gen-world`, see
docs/WORLD_MAP_ARCHITECTURE.md) plus the classic World map. The registry
is
[`src/core/configuration/SoloMaps.ts`](../src/core/configuration/SoloMaps.ts):

```ts
export const ENABLED_SOLO_MAPS: readonly GameMapType[] = [
  GameMapType.WorldOceania, // one continuous Oceania — default
  GameMapType.World,
];
```
