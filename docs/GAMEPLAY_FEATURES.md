...
| 5   | **Rivers**             | Rivers are navigable **estuaries**: the lower ~150 km of every major river (plus hand-added Yarra, Brisbane, Swan, Derwent, Chao Phraya, Saigon, Pasig) is a 3-tile-wide ocean-connected channel ships can sail up. Upstream they never obstruct land expansion — no thin water lines across the interior. | pipeline (`paintRivers`)                    |
...
| 14  | **Capital crisis**     | Your first spawn tile is your capital — humans and nations get a free capital City there, marked by an imposing **castle** (every player's capital gets one; yours is bigger, gold-trimmed and labelled). Losing it halves gold income until you retake it; after ~1 min a small realm relocates its government, but a large empire **shatters**: 2–3 AI successor states split off and only a rump around a surviving city remains yours. | `PlayerExecution` + `ResourceOverlay`       |
...
| —   | **Overextension**    | Blitzing new territory builds **unrest** (0–100). A warning fires at 50; at 100 a border region **declares independence as a new AI rebel state** — announced with an expanding ripple in the rebels' colour as the map recolours. Garrisoning (keeping >50% of max troops in reserve) halves unrest gains, and the radial **Invest** action buys it down directly. | `PlayerExecution` + `StabilizeExecution` |
...
| —   | **Teams/coalitions** | Solo setup has a Game Mode section: Free-for-All or 2–6 coalitions. You and the AI nations are split into teams (shared victory, no friendly fire) using the engine's team machinery.                                                                                   | `SinglePlayerModal` + `GameImpl`      |
| —   | **Southeast Asia**   | A second one-world theatre: Burma, Thailand, Indochina, Malaya, Sumatra/Borneo and the whole Philippines in ONE ~21M-tile map (~1.2 km/tile), with monsoon climate, 9 real resource sites, and the Malacca/Sunda/Luzon straits joining the chokepoint roster. Navigable Irrawaddy, Chao Phraya (into Bangkok), Mekong and Saigon estuaries. | pipeline + `SoloMaps`                 |
